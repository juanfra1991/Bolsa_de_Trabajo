...
<title>Home Añadir Ofertas</title>

<script type="text/javascript">  
function Valida(form) { 
    if (form.oferta_valida.value == 'ok') {
        return true;
    } 
    else {
...
function comprobar() {
var cod_oferta = document.getElementById("cod_oferta").value;
var no_vacio = document.forms["form"]["cod_oferta"].value;
    if (no_vacio == "") {
        if ($(".mensaje").hasClass('alert-success') || $(".mensaje").hasClass('alert-danger')) {
            $(".mensaje").removeClass('alert-success');
            $(".mensaje").removeClass('alert-danger');
        }
        $(".mensaje").addClass('alert-warning').toggleClass('hide').addClass('show').html("<button type='button' class='close' data-dismiss='alert'>&times;</button><strong>Aviso!</strong> El campo 'Código Oferta' no puede estar vacío");
        return false;
    }
peticion_http = inicializa_xhr();
    if (peticion_http) {
        peticion_http.onreadystatechange = procesaRespuesta;
        peticion_http.open("POST", "check_oferta.jsp", true);
        peticion_http.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        peticion_http.send("cod_oferta="+cod_oferta);
    }
}
    ...
    if (peticion_http.readyState == READY_STATE_COMPLETE) {
        if (peticion_http.status == 200) {
            ...
            if (estado_respuesta=="Si") {
                ...
                $(".mensaje").addClass('alert-success').toggleClass('hide').addClass('show').html("<button type='button' class='close' data-dismiss='alert'>&times;</button><strong>Aviso!</strong> El Código Oferta '"+cod_oferta+"' está disponible");
                    document.getElementById("oferta_valida").value="ok";
            }
            else {
            ...
            $(".mensaje").addClass('alert-danger').toggleClass('hide').addClass('show').html("<button type='button' class='close' data-dismiss='alert'>&times;</button><strong>Aviso!</strong> El Código Oferta '"+cod_oferta+"' no está disponible");
                document.getElementById("oferta_valida").value="no_ok";
            }
        }
    }
}
...
<nav class="navbar navbar-expand-lg navbar-dark bg-dark">
    <a class="navbar-brand" href="index_anadir_ofertas.jsp">Bolsa de Trabajo</a>
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
        ...
        <!-- Dropdown -->
            ...
            <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle active" href="#" id="navbardrop" data-toggle="dropdown">Ofertas </a>
                <div class="dropdown-menu">
                    <a class="dropdown-item" href="index_lista_ofertas.jsp">Lista Ofertas</a>
                    <a class="dropdown-item" href="index_anadir_ofertas.jsp">Añadir Ofertas</a>
                </div>
            </li>
            <li class="nav-item dropdown">
...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-6">
            <h2 class="text-center">Añadir Ofertas</h2>
            <hr>
        </div>
    </div>
    <form class="form-horizontal" name="form" id="form" role="form" method="POST" action="anadir_ofertas.jsp" onSubmit="return Valida(this);">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <div class="input-group mb-2 mr-sm-2 mb-sm-0">
                        ...
                        <input type="text" name="cod_oferta" class="form-control" id="cod_oferta" placeholder="Código Oferta *" required autofocus>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <div class="input-group mb-2 mr-sm-2 mb-sm-0">
                        ...
                        <textarea class="form-control" name="descripcion" cols="4" rows="8"></textarea>
                    </div>
                </div>
            </div>
        </div>
        <select style="display: none;" name="email">
        <%
        String urljdbc;
        String loginjdbc;
        String passwordjdbc;
        ...
        try {
            ...
            while(sentencia_sql.next()) {
                out.println("<option>"+sentencia_sql.getString(1)+"</option>");
            }
            sentencia_sql.close();
            sentencia.close();
            conexion.close();
            ...
            <div class="col-md-6">
                <div class="form-group">
                    <div class="input-group mb-2 mr-sm-2 mb-sm-0">
                        ...
                        <select name="cif" class="form-control" id="cif" required>
                            ...
                            try {
                                ...
                                conexion = DriverManager.getConnection(urljdbc,loginjdbc,passwordjdbc);
                                sentencia=conexion.createStatement(ResultSet.TYPE_FORWARD_ONLY,ResultSet.CONCUR_READ_ONLY);
                                sentencia_sql=sentencia.executeQuery("select * from empresa order by nombre");            
                                // Bucle para recorrer el resultado del select
                                while(sentencia_sql.next()) {
        ...
        <div align='center' style='padding-top: 1rem'>
            <input type="hidden" name="oferta_valida" id="oferta_valida" value="no_ok">
